...
"""
Training script for RNN (GRU) Baseline using Reinforcement Learning.
Adapts scripts/train_ann_baseline.py for a recurrent neural network architecture.

Key differences from SNN training:
- Uses 80 input channels directly (no downsampling constraint)
- Uses GRU temporal sequence processing instead of spike integration or mean-pooling
- Standard continuous/discrete activations in conventional hardware

All other aspects (environment, reward, hyperparameters, curriculum learning)
are matched exactly to the SNN and ANN training for fair comparison.
"""
...
from src.utils.rnn_utils import select_action_rnn, optimize_model_rnn
from src.utils.io_utils import save_results, save_checkpoint, save_config, load_checkpoint

# Hyperparameters (Matched to train_ann_baseline.py & train_rl_rockpool_16ch.py)
BATCH_SIZE = 128
GAMMA = 0.99
EPS_START = 0.9
EPS_END = 0.05
...
def train(policy_net, target_net, memory, optimizer, env, num_episodes, save_freq, device,
    ...
    """
    Main training loop — mirrors train_ann_baseline.py structure exactly.
    """
    ...
    for i_episode in tqdm(range(start_episode, num_episodes)):
        # Curriculum Learning Logic (identical to SNN/ANN training)
        if curriculum:
        ...
        # Prepare state: (T, 80) -> (1, T, 80) — RNN uses full 80 channels with temporal history
        ...
        for t in tqdm(range(env.max_steps), desc=f"Episode {i_episode + 1}/{num_episodes}", leave=False):
            ...
            action = select_action_rnn(policy_net, state, env, eps_steps, device,
            ...
            loss = optimize_model_rnn(policy_net, target_net, optimizer, memory, device,
                                      BATCH_SIZE, GAMMA)
            ...
            # Soft update target network (same as SNN/ANN)
        ...
        if (i_episode + 1) % save_freq == 0:
            checkpoint_dir = 'models/checkpoints_rnn_baseline'
            if curriculum:
            ...
            results_dir = 'data/results_rnn_baseline'
            if curriculum:
...
def main():
    ...
    Example usage (matching the SNN/ANN command):
    python -m scripts.train_rnn_baseline --num-episodes 500 --curriculum --delta 1.0 --healthy-duration 100
    """
    parser = argparse.ArgumentParser(description="Train an RNN baseline model for DBS optimization.")
    ...
    parser.add_argument('--max-steps', type=int, default=50,
                        help='Max steps per episode. V2 baseline: 100 steps.')
    ...
    project_name = "cl-dbs-rl-rnn-baseline"
    if args.curriculum:
        project_name += "-curriculum"

    # Select environment class based on --legacy-reward flag
    # Identical to train_ann_baseline.py and train_rl_rockpool_16ch.py
    ...
    # RNN uses full 80 channels (no Xylo downsampling constraint)
    ...
    print('Training RNN Baseline (matched to SNN/ANN environment)')
    print('=' * 40)
    ...
    # --- Build Networks ---
    policy_net = RNNBaseline(n_observations, num_hidden, n_actions).to(device)
    target_net = RNNBaseline(n_observations, num_hidden, n_actions).to(device)
    ...
    if args.checkpoint:
        ...
        if args.curriculum:
            intermediate_results_path = 'data/results_rnn_baseline_curriculum/intermediate_results.pth'
        else:
            intermediate_results_path = 'data/results_rnn_baseline/intermediate_results.pth'
    ...
    memory = ReplayMemory(100000)  # Matched to SNN/ANN training
    ...
    results_dir = 'data/results_rnn_baseline'
    models_dir = 'models/rnn_baseline'
    configs_dir = 'configs_rnn_baseline'
    ...
    config = {
        ...
        'env_class': env.__class__.__name__, 'model_type': 'RNN',
    }
    save_config(config, os.path.join(configs_dir, config_filename))

    print("\nTraining completed.")
    print(f"RNN baseline model saved to: {os.path.join(models_dir, model_filename)}")
