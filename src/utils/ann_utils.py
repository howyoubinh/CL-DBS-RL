...
def select_action_ann(policy_net, state, env, steps_done, device, eps_start, eps_end, eps_decay):
    """
    Select action for ANN using epsilon-greedy policy.
    """
...
def flatten_action_ann(actions, device):
    """
    Flatten action dictionaries to tensor for ANN optimization.
    Same logic as action_utils.flatten_action but standalone to avoid circular deps if any.
    """
    action_indices = []
...
def optimize_model_ann(policy_net, target_net, optimizer, memory, device, batch_size, gamma):
    """
    Perform one step of optimization on the ANN policy network.
    """
    ...
    # Forward pass
    # state_batch: [Batch, Time, Channels] -> ANN averages time internally
    state_action_values_full = policy_net(state_batch) # [Batch, 9]
    
    # Prepare actions
    action_indices_flat = flatten_action_ann(batch.action, device) # [Batch, 3]
